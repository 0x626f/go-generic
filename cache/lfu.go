...
import (
	"github.com/0x626f/go-generic/linkedlist"
	"github.com/0x626f/go-generic/utils"
)
...
type LFUCache[K comparable, D any] struct {
	...
	frequencies *linkedlist.LinkedList[*record[uint, PrimaryCache[K, D]]]

	// data maps frequency counts to their corresponding nodes in the frequencies list
	data PrimaryCache[uint, *linkedlist.LinkedNode[*record[uint, PrimaryCache[K, D]]]]

	// spot maps keys to their frequency bucket nodes for O(1) lookup
	spot PrimaryCache[K, *linkedlist.LinkedNode[*record[uint, PrimaryCache[K, D]]]]
}
...
func NewLFUCache[K comparable, D any](capacity int) *LFUCache[K, D] {
	return &LFUCache[K, D]{
		capacity:    capacity,
		frequencies: linkedlist.NewLinkedList[*record[uint, PrimaryCache[K, D]]](),
		data:        make(PrimaryCache[uint, *linkedlist.LinkedNode[*record[uint, PrimaryCache[K, D]]]]),
		spot:        make(PrimaryCache[K, *linkedlist.LinkedNode[*record[uint, PrimaryCache[K, D]]]]),
	}
}
...
func (cache *LFUCache[K, D]) record(freq uint) *linkedlist.LinkedNode[*record[uint, PrimaryCache[K, D]]] {
	if cache.data[freq] == nil {
		entry := &record[uint, PrimaryCache[K, D]]{key: freq, value: make(PrimaryCache[K, D])}
...
func (cache *LFUCache[K, D]) Get(key K) (D, bool) {
	...
	nextNode := cache.record(node.Data.key + 1)

	nextNode.Data.value[key] = node.Data.value[key]

	delete(node.Data.value, key)
	cache.spot[key] = nextNode

	return nextNode.Data.value[key], true
}
...
func (cache *LFUCache[K, D]) Flush() {
	if cache.frequencies.Size() > cache.capacity {
		cache.frequencies.Sort(func(arg0, arg1 *record[uint, PrimaryCache[K, D]]) int {
			return int(arg1.key) - int(arg0.key)
		})
		cache.frequencies.ForEach(func(index int, data *record[uint, PrimaryCache[K, D]]) bool {
			if (index + 1) > cache.capacity {
				for key := range data.value {
					delete(cache.spot, key)
				}
