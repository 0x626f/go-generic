...
import (
	"github.com/0x626f/go-generic/abstract"
...
type Set[I comparable, T abstract.Keyable[I]] struct {
	data map[I]T
}

// New creates and returns an empty Set instance.
func New[I comparable, T abstract.Keyable[I]]() *Set[I, T] {
...
func Wrap[I comparable, T abstract.Keyable[I]](items ...T) *Set[I, T] {
	instance := New[I, T]()
...
func (set *Set[I, T]) Join(collection abstract.Collection[I, T]) {
...
func (set *Set[I, T]) Merge(collection abstract.Collection[I, T]) abstract.Collection[I, T] {
	result := New[I, T]()
...
func (set *Set[I, T]) DeleteBy(predicate abstract.Predicate[T]) {
...
func (set *Set[I, T]) Some(predicate abstract.Predicate[T]) bool {
...
func (set *Set[I, T]) Find(predicate abstract.Predicate[T]) (T, bool) {
	for _, item := range set.data {
		if predicate(item) {
			return item, true
		}
...
func (set *Set[I, T]) Filter(predicate abstract.Predicate[T]) abstract.Collection[I, T] {
	result := New[I, T]()
...
func (set *Set[I, T]) ForEach(receiver abstract.IndexedReceiver[I, T]) {
	for index, item := range set.data {
